/**
 * CHINTAI scraper parse tests (standalone Node runner — no vitest dependency).
 *
 * Run:
 *   npx tsx tests/scrapers/chintai.test.ts
 */
...
const FIXTURE_PATH = path.resolve(__dirname, '../fixtures/chintai-listings.html');
...
console.log('\n[chintai-parser] extractJsonLd — item count');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
...
console.log('\n[chintai-parser] findJsonLdByType — RealEstateListing nodes');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
  const items = extractJsonLd(doc);
...
console.log('\n[chintai-parser] first listing — name field');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
  const items = extractJsonLd(doc);
...
console.log('\n[chintai-parser] first listing — monthly rent price field');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
  const items = extractJsonLd(doc);
  ...
  // CHINTAI stores monthly rent in JPY (e.g. 180000 = 18万円/月)
  assert(!isNaN(price) && price > 0, `rent price is a positive number (${price})`);
}

console.log('\n[chintai-parser] first listing — floorSize');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
  const items = extractJsonLd(doc);
...
console.log('\n[chintai-parser] first listing — image array');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
  const items = extractJsonLd(doc);
...
console.log('\n[chintai-parser] second listing — image string');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
  const items = extractJsonLd(doc);
...
console.log('\n[chintai-parser] first listing — address locality');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
  const items = extractJsonLd(doc);
...
console.log('\n[chintai-parser] first listing — geo coordinates');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
  const items = extractJsonLd(doc);
...
console.log('\n[chintai-parser] parseDocument on empty string');
{
  parseDocument('');
  assert(true, 'parseDocument does not throw on empty string');
}

console.log('\n[chintai-parser] extractJsonLd — malformed JSON-LD skipped');
{
