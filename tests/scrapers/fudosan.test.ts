/**
 * 不動産Japan scraper parse tests (standalone Node runner — no vitest dependency).
 *
 * Run:
 *   npx tsx tests/scrapers/fudosan.test.ts
 */
...
const FIXTURE_PATH = path.resolve(__dirname, '../fixtures/fudosan-listings.html');
...
console.log('\n[fudosan-parser] extractJsonLd — item count');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
...
console.log('\n[fudosan-parser] findJsonLdByType — RealEstateListing nodes');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
  const items = extractJsonLd(doc);
...
console.log('\n[fudosan-parser] first listing — name field');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
  const items = extractJsonLd(doc);
...
console.log('\n[fudosan-parser] first listing — price field');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
  const items = extractJsonLd(doc);
...
console.log('\n[fudosan-parser] first listing — floorSize');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
  const items = extractJsonLd(doc);
...
console.log('\n[fudosan-parser] first listing — image array');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
  const items = extractJsonLd(doc);
...
console.log('\n[fudosan-parser] second listing — image string');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
  const items = extractJsonLd(doc);
...
console.log('\n[fudosan-parser] first listing — address locality');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
  const items = extractJsonLd(doc);
...
console.log('\n[fudosan-parser] first listing — geo coordinates');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
  const items = extractJsonLd(doc);
...
console.log('\n[fudosan-parser] parseDocument on empty string');
{
  parseDocument('');
  assert(true, 'parseDocument does not throw on empty string');
}

console.log('\n[fudosan-parser] extractJsonLd — malformed JSON-LD skipped');
{
