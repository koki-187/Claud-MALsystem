/**
 * Smaity scraper parse tests (standalone Node runner — no vitest dependency).
 *
 * Run:
 *   npx tsx tests/scrapers/smaity.test.ts
 */
...
const FIXTURE_PATH = path.resolve(__dirname, '../fixtures/smaity-listings.html');
...
console.log('\n[smaity-parser] extractJsonLd — item count');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
...
console.log('\n[smaity-parser] findJsonLdByType — RealEstateListing nodes');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
  const items = extractJsonLd(doc);
...
console.log('\n[smaity-parser] first listing — name field');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
  const items = extractJsonLd(doc);
...
console.log('\n[smaity-parser] first listing — price field');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
  const items = extractJsonLd(doc);
...
console.log('\n[smaity-parser] first listing — floorSize');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
  const items = extractJsonLd(doc);
...
console.log('\n[smaity-parser] first listing — image array');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
  const items = extractJsonLd(doc);
...
console.log('\n[smaity-parser] second listing — image string');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
  const items = extractJsonLd(doc);
...
console.log('\n[smaity-parser] first listing — additionalProperty yield rate');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
  const items = extractJsonLd(doc);
  const nodes = findJsonLdByType(items, 'RealEstateListing');
  const additional = nodes[0]['additionalProperty'];
  assert(Array.isArray(additional) && (additional as unknown[]).length > 0, 'first node has additionalProperty');
  const yieldProp = (additional as Record<string, unknown>[]).find(p => p['name'] === '表面利回り');
  assert(yieldProp !== undefined, 'additionalProperty contains 表面利回り');
  const yieldVal = parseFloat(String(yieldProp?.['value'] ?? ''));
  assert(!isNaN(yieldVal) && yieldVal > 0, `yield rate is positive (${yieldVal})`);
}

console.log('\n[smaity-parser] first listing — address locality');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
  const items = extractJsonLd(doc);
...
console.log('\n[smaity-parser] first listing — geo coordinates');
{
  const html = fs.readFileSync(FIXTURE_PATH, 'utf-8');
  const doc = parseDocument(html)!;
  const items = extractJsonLd(doc);
...
console.log('\n[smaity-parser] parseDocument on empty string');
{
  parseDocument('');
  assert(true, 'parseDocument does not throw on empty string');
}

console.log('\n[smaity-parser] extractJsonLd — malformed JSON-LD skipped');
{
